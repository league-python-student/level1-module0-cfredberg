...
def add_guest(list_of_guests):
    name = simpledialog.askstring("Add Guest","Guest name:")
...
def remove_guest(list_of_guests):
    name = simpledialog.askstring("Remove Guest", "Guest Name:")
    if list_of_guests.count(name) != 0:
        list_of_guests.remove(name)
    return list_of_guests
...
def print_guests(list_of_guests):
    ...
    for i in range(len(list_of_guests)):
        displayList = displayList + "Guest " + str(i+1) + ". " + list_of_guests[i] + "\n"
    messagebox.showinfo("Guest List", displayList)
    pass
